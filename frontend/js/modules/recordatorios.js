        ...
        return `
          <label style="display:flex;align-items:center;gap:12px;padding:12px 20px;border-bottom:1px solid var(--border);cursor:pointer"
            ...
            <div style="flex:1;min-width:0">
              ...
              <div style="font-size:.75rem;color:var(--muted)">${c.Correo || 'Sin correo'} · Día ${c.Fecha_Pago}</div>
            </div>
            <div style="text-align:right;flex-shrink:0">
              <div style="font-weight:700;font-family:'Sora',sans-serif;color:var(--error);font-size:.88rem">$${saldo.toLocaleString('es-MX', { minimumFractionDigits: 2 })}</div>
...
export async function enviarRecordatorios() {
    ...
    try {
        const res  = await fetch(`${API_BASE_URL}/api/recordatorios/enviar`, {
            ...
            body   : JSON.stringify({ ids_clientes: ids, id_usuario: usuario?.id })
        });
        const data = await res.json();
        if (data.success) {
        ...
        if (btn) { btn.innerHTML = 'Enviar correos'; btn.disabled = false; }
    }
}
